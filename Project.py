...
# Welcome words
print("\nWelcome to point cloud rotator.")
...
working_directory = input("Path: ").strip()
...
while True:
    ...
    while True:
        ...
        if choise == "G" or choise == "g":
            ...
            matrix =  " ".join(map(str, (a11, a12, a13, a14, "\n")))
            matrix += " ".join(map(str, (a21, a22, a23, a24, "\n")))
            matrix += " ".join(map(str, (a31, a32, a33, a34, "\n")))
            matrix += " ".join(map(str, (a41, a42, a43, a44)))
            ...
            with open(join(working_directory, f"{matrix_name}.txt"), "w") as matrix_file:
        ...
        # If we choosing Rotation, new text file with rotated point cloud will appear
            ...
            cloud_file_name = input(": ") + ".txt"
            print("Preparing point cloud...")

            # Counting lines in initial file
            with open(join(working_directory, cloud_file_name), "r") as initial_cloud:
                lines_total = sum(1 for line in initial_cloud)

            # Creating a new file and writing new coordinates of rotated point cloud in it 
            with open(join(working_directory, cloud_file_name), "r") as initial_cloud:
                with open(join(working_directory, f"{cloud_file_name[:-4]}_ROTATED.txt"), "w") as rotated_cloud:
                    print("Rotating...")
                    lines_counter = 0
                    print("0%")
                    for line in initial_cloud:
                        ...
                        new_line = f"{x2:.5f} {y2:.5f} {z2:.5f} {attribute}\n"
            ...
            print("100%")
            print("Finished")
            print(f"{lines_counter} points processed")
            break
